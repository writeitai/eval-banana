...
| `--harness-timeout-seconds INT` | Positive per-agent wall-clock limit for each `harness_judge` subprocess (default `300`). |
| `--max-parallel-checks INT` | Positive number of checks run concurrently (default `4`). Report ordering stays deterministic regardless. |
