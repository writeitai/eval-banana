...
`stdout` and `stderr` are captured and written to `<output_dir>/checks/<check_id>.stdout.txt` and `.stderr.txt` (only when non-empty). Print a short explanation to `stderr` before exiting non-zero so the report says *why* a check failed:
