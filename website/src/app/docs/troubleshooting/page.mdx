...
**The judge returns prose instead of JSON.** The runner requires a strict `{"score": 0|1, "reason": "..."}` final output. Anything else — prose, a score outside `{0, 1}`, malformed JSON — is an `error`, which sinks the run. Tell the agent, in the `instructions`, to respond with JSON only.
