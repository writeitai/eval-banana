...
| `max_parallel_checks` | `4` | `EVAL_BANANA_MAX_PARALLEL_CHECKS` | Positive number of checks run concurrently. Each `harness_judge` check is its own subprocess, so parallelism cuts wall-clock time; report ordering stays deterministic regardless. Also settable per run with `--max-parallel-checks`. |
...
| `EVAL_BANANA_MAX_PARALLEL_CHECKS` | `max_parallel_checks` |
