...
The console prints the run id, `points_earned/total_points` with a percentage, a `PASS`/`FAIL` verdict, and a per-check breakdown. The process exits `0` when the run passes and `1` otherwise, so `eb run` drops straight into CI. See [Reports](/docs/reports).
