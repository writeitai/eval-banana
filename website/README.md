# eval-banana docs site

The public documentation for [eval-banana](https://github.com/writeitai/eval-banana),
...
2. **Settings → Pages → Custom domain:** enter `eval-banana.writeit.ai` and save (this
   is what actually binds the domain; the committed `CNAME` file just records the
   intent).
3. **DNS** (in the `writeit.ai` zone): add
   `eval-banana.writeit.ai CNAME writeitai.github.io.`
...
`https://writeitai.github.io/eval-banana/`, where the root-relative `/_next/` and
