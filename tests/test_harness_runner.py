...
def test_run_harness_calls_distribute_skills(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    calls: list[str] = []
    ...
    monkeypatch.setattr(
        "eval_banana.harness.runner.distribute_skills", fake_distribute_skills
    )
    monkeypatch.setattr("eval_banana.harness.runner.subprocess.run", fake_run)
    ...
    assert calls == ["distribute", "subprocess"]


def test_run_harness_forwards_skills_dir(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    captured: dict[str, object] = {}
    ...
    assert captured["skills_dir"] == skills_dir


def test_run_harness_skills_dir_none_skips_distribution(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(
        "eval_banana.harness.runner.distribute_skills",
        lambda **kwargs: (_ for _ in ()).throw(
            AssertionError("distribute_skills must not be called")
        ),
    )
    monkeypatch.setattr(
        "eval_banana.harness.runner.subprocess.run",
